...
def inicializar_db():
    ...
    # progreso separado por dificultad
...
def obtener_progreso(gid, dificultad="Normal"):
    ...
    row = cur.fetchone()
    if not row:
        cur.execute("""INSERT INTO progreso_juego
...
def partida_en_curso(gid, dificultad="Normal"):
    """Devuelve True si hay una partida sin terminar en esa dificultad."""
    con = _conn(); cur = con.cursor()
...
def obtener_cooldowns(gid, dificultad="Normal"):
    con = _conn(); cur = con.cursor()
    cur.execute("SELECT decision,disponible_en FROM cooldown_decisiones WHERE grupo_id=? AND dificultad=?",
                (gid, dificultad))
    rows = cur.fetchall(); con.close()
    return {r["decision"]: r["disponible_en"] for r in rows}
...
def actualizar_cooldown(gid, decision, ronda_usada, dificultad="Normal"):
    ...
    cur.execute("INSERT INTO cooldown_decisiones(grupo_id,dificultad,decision,disponible_en) VALUES(?,?,?,?)",
...
def obtener_ranking(dificultad=None):
    con = _conn(); cur = con.cursor()
    if dificultad:
        cur.execute("""SELECT r.*, r.grupo_id FROM ranking r
            WHERE r.dificultad=? ORDER BY r.puntaje DESC LIMIT 20""", (dificultad,))
    else:
        cur.execute("""SELECT r.*, r.grupo_id FROM ranking r
            ORDER BY r.puntaje DESC LIMIT 20""")
    rows = cur.fetchall(); con.close()
