...
# Tiempo de countdown en pantallas de resultado y evento
TIEMPO_RESULTADO = 4
TIEMPO_EVENTO    = 5
...
# ══════════════════════════════════════════════════════════════════════════════
#  ENCABEZADO — con estrellas acumuladas desde database.db
# ══════════════════════════════════════════════════════════════════════════════
...
def _cabecera(nombre_grp, estudiantes, ronda, est_turno, dif, ind, estrellas):
    ...
    for i in range(1, TOTAL_RONDAS + 1):
        if i < ronda:
            ...
            bg_r = "transparent"; col_r = "rgba(255,255,255,.2)"; brd_r = "rgba(255,255,255,.1)"
        ronda_chips += (
    ...
    for e in estudiantes:
        if e == est_turno:
            est_chips += (
                ...
                "box-shadow:0 0 12px rgba(167,139,250,.3)'>✏️ " + e + "</span>")
        else:
            est_chips += (
    ...
    for key in ["economia", "medio_ambiente", "energia", "bienestar_social"]:
        ...
        crit = v < 30
        ring = ("border:1.5px solid #ef444466;" if crit else "border:1px solid " + color + "18;")
        pulse = ("<div style='position:absolute;top:4px;right:4px;width:7px;height:7px;"
                 "border-radius:50%;background:#ef4444;box-shadow:0 0 6px #ef4444'></div>"
                 if crit else "")
        ind_html += (
            ...
            "<div style='margin-top:4px;background:rgba(255,255,255,.06);"
            "border-radius:3px;height:3px'>"
            "<div style='width:" + str(v) + "%;background:" + color + ";"
            "height:3px;border-radius:3px'></div></div></div>")
    ...
    with col_hdr:
        st.markdown(
            "<style>"
            "@keyframes hglow{0%,100%{box-shadow:0 0 20px " + dm["color"] + "18,0 8px 32px rgba(0,0,0,.5)}"
            "50%{box-shadow:0 0 40px " + dm["color"] + "30,0 8px 32px rgba(0,0,0,.5)}}"
            ...
            "animation:hglow 3s ease-in-out infinite;position:relative;overflow:hidden'>"
            ...
            + dm["color"] + ",transparent);animation:sweep 2s linear infinite'></div></div>"

            # Fila 1: nombre + dif + fase + ⭐ estrellas desde BD
            ...
            "border:1px solid " + dm["color"] + "50;border-radius:20px;"
            ...
            # Estrellas acumuladas — siempre visible, leídas desde database.db
            ...
            "box-shadow:0 0 10px rgba(251,191,36,.20)'>⭐ " + str(estrellas) + "</span>"
            "</div></div>"
            ...
            "border-radius:4px;box-shadow:0 0 10px " + dm["color"] + "80'></div></div>"
            "</div>",
...
# ══════════════════════════════════════════════════════════════════════════════
#  PANEL DE ATRIBUTOS — canjear estrellas acumuladas en database.db
# ══════════════════════════════════════════════════════════════════════════════
...
def _panel_estrellas(gid, estrellas):
    ...
    st.markdown(
        ...
        "padding:2px 10px;font-size:.70rem;font-weight:800'>"
    ...
    for i, (key, atr) in enumerate(ATRIBUTOS.items()):
        ...
        with col:
            st.markdown(
                ...
                "color:#f1f5f9;line-height:1.2;margin-bottom:2px'>" + atr["nombre"] + "</div>"
                ...
                "padding:1px 4px;font-size:.46rem'>" + atr["tipo"].upper() + "</span>"
                "<span style='color:#fbbf24;font-weight:700;font-size:.62rem'>"
                "⭐" + str(atr["costo"]) + "</span>"
                "</div></div>",
                unsafe_allow_html=True)
            ...
            if activo:
                ...
                if st.button("+" + str(atr["costo"]) + "⭐",
                             key="atr_" + key, use_container_width=True):
                    # Restar estrellas en database.db
...
def pantalla_juego():
    ...
    # Leer estrellas acumuladas desde database.db en cada ciclo
    ...
    # ══ FASE DECISIÓN ═════════════════════════════════════════════════════════
    if fase == "decision":
        # Panel de atributos con estrellas desde database.db
        ...
        for i, (nom_dec, ef) in enumerate(DECISIONES.items()):
            ...
            with col:
                ...
                if st.button("Elegir" if disp else "Bloqueada",
                             disabled=not disp, key="dec_" + nom_dec,
    ...
    # ══ FASE PREGUNTA — countdown automático ══════════════════════════════════
        ...
        st.markdown(
            ...
            " <span style='color:rgba(255,255,255,.22);font-size:.68rem'>" + ef_resumen + "</span>"
        ...
        st.markdown(
            ...
            + alerta + "</div></div></div>", unsafe_allow_html=True)

        # Pregunta
        cat_map = {"Python":"#6366f1","PSeInt":"#8b5cf6","Calculo":"#06b6d4",
                   "Derivadas":"#10b981","Fisica MRU":"#f59e0b","Fisica MRUA":"#ef4444",
                   "Matrices":"#ec4899","Logica":"#f97316","Algebra":"#84cc16",
                   "Estadistica":"#a78bfa","Sistemas":"#34d399"}
        cat_color = cat_map.get(pregunta.get("cat",""), "#a78bfa")
        dif_col_m = {"facil":"#10b981","normal":"#f59e0b","dificil":"#ef4444"}
        dif_lbl_m = {"facil":"FÁCIL","normal":"NORMAL","dificil":"DIFÍCIL"}
        dif_col = dif_col_m.get(pregunta.get("dif","normal"), "#a78bfa")
        dif_lbl = dif_lbl_m.get(pregunta.get("dif","normal"), "")
        st.markdown(
            ...
            "font-size:.63rem;font-weight:700'>" + pregunta.get("cat","") + "</span>"
        ...
        if correcta:
            ...
            if _atributo_activo("escudo_ciudad"):
                ...
                st.markdown(
                    ...
                    "🛡️ Escudo: " + str(penaliz) + " → " + str(penaliz_f) + " pts</div>",
                    unsafe_allow_html=True)
            nuevo_ind = {k: _clamp(v - penaliz_f) for k, v in ind.items()}
            prot_map  = {"economia":"prot_economia","medio_ambiente":"prot_ambiente",
                         "energia":"prot_energia","bienestar_social":"prot_bienestar"}
            for ik, ak in prot_map.items():
                if _atributo_activo(ak):
                    nuevo_ind[ik] = ind[ik]
                    activos = st.session_state.get("atributos_activos", set())
                    activos.discard(ak)
                    st.session_state["atributos_activos"] = activos
            ...
            icono_r  = "⏱️" if agotado else "❌"
            titulo_r = "Tiempo Agotado" if agotado else "Respuesta Incorrecta"
            st.markdown(
                "<div style='background:rgba(239,68,68,.07);"
                "border:1px solid rgba(239,68,68,.22);"
                "border-radius:16px;padding:22px;text-align:center;margin-bottom:12px'>"
                "<div style='font-size:2.8rem;margin-bottom:5px'>" + icono_r + "</div>"
                "<h3 style='color:#f87171;margin:0 0 4px;font-size:1.25rem'>" + titulo_r + "</h3>"
    ...
    # ══ FASE EVENTO — countdown automático ════════════════════════════════════
        ...
        st.markdown(
            ...
            "</div>" + escudo_msg + "</div>", unsafe_allow_html=True)
