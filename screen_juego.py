...
from config import (DECISIONES, EVENTOS, TOTAL_RONDAS, COOLDOWN, TIEMPO_PREGUNTA,
                    IND_COLOR, IND_LABEL, MEZCLA_PREGUNTAS, DIFICULTADES, PREGUNTAS)
...
def _seleccionar_pregunta():
    ...
    mezcla = MEZCLA_PREGUNTAS.get(dif, MEZCLA_PREGUNTAS["Normal"])
    ...
    # Intentar respetar la mezcla de dificultad
    niveles_orden = sorted(mezcla.keys(), key=lambda k: mezcla[k], reverse=True)
    for nivel in niveles_orden:
        candidatos = [i for i in disponibles if PREGUNTAS[i].get("dif") == nivel]
        if candidatos:
...
def _barra_indicador(nombre, valor, emoji):
    ...
    if valor >= 60:
        ...
        bg_card = "rgba(16,185,129,0.08)"
        borde   = "rgba(16,185,129,0.3)"
    elif valor >= 30:
        color, badge = "#f59e0b", "Precaución"
        bg_card = "rgba(245,158,11,0.08)"
        borde   = "rgba(245,158,11,0.3)"
    else:
        color, badge = "#ef4444", "Crítico"
        bg_card = "rgba(239,68,68,0.1)"
        borde   = "rgba(239,68,68,0.35)"
...
def _cabecera(nombre_grp, estudiantes, ronda, est_turno, ver_instrucciones):
    ...
    with top2:
        # Menú sin expander — botones directos
...
def _panel_instrucciones():
    st.markdown(
        ...
        "font-family:Courier Prime,monospace;padding-left:18px'>"
...
def pantalla_juego():
    ...
    _cabecera(nombre_grp, estudiantes, ronda, est_turno,
              st.session_state.get("_ver_instrucciones", False))
    ...
    with m1: st.metric("Ronda",    str(ronda) + "/" + str(TOTAL_RONDAS))
    with m2: st.metric("Turno",    est_turno)
    with m3: st.metric("Progreso", str(pct) + "%")
    with m4: st.metric("Fase",     fase_txt.get(fase_label, fase_label))

    st.markdown(
        "<div style='background:rgba(255,255,255,0.07);border-radius:4px;height:5px;margin:2px 0 14px'>"
        "<div style='width:" + str(pct) + "%;background:linear-gradient(90deg,#a78bfa,#60a5fa);"
    ...
    if st.session_state.get("_ver_instrucciones", False):
        _panel_instrucciones()
        return

    # Indicadores
    ci1, ci2, ci3, ci4 = st.columns(4)
    with ci1: _barra_indicador("Economía",      ind["economia"],        "💰")
    with ci2: _barra_indicador("Medio Ambiente", ind["medio_ambiente"],  "🌿")
    with ci3: _barra_indicador("Energía",        ind["energia"],         "⚡")
    with ci4: _barra_indicador("Bienestar",      ind["bienestar_social"],"❤️")
        ...
        for i, (nom_dec, ef) in enumerate(DECISIONES.items()):
            ...
            for k, v in ef.items():
                ...
                filas_ef += (
                    ...
                    "<span style='color:" + col_ind + ";font-size:.74rem'>" + em_ind + " " + IND_LABEL.get(k,"") + "</span>"
            ...
            with col:
                ...
                if st.button("ELEGIR", disabled=not disp,
    ...
    # ═════════════════════════ FASE: PREGUNTA ═════════════════════════════════
        ...
        tiempo_transc   = time.time() - timer_ini
        tiempo_restante = max(0.0, TIEMPO_PREGUNTA - tiempo_transc)
        pct_timer       = tiempo_restante / TIEMPO_PREGUNTA

        if pct_timer > 0.5:   col_timer = "#10b981"
        elif pct_timer > 0.25: col_timer = "#f59e0b"
        else:                  col_timer = "#ef4444"
        seg = int(tiempo_restante)

        # Auto-avanzar si se agota
        ...
        # Decisión elegida resumen
        ef_resumen = " · ".join(
            IND_COLOR.get(k, ("#94a3b8", ""))[1] + (" +" if v > 0 else " ") + str(v)
        ...
        st.markdown(
            ...
            "<span style='color:rgba(255,255,255,.35);font-size:.76rem;font-family:Courier Prime,monospace'>"
            + ef_resumen + "</span></div>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            ...
            "<span style='color:rgba(255,255,255,.5);font-size:.82rem;font-family:Courier Prime,monospace'>"
            "⏱ Tiempo restante</span>"
            "<span style='color:" + col_timer + ";font-weight:900;font-size:1.6rem;font-variant-numeric:tabular-nums'>"
            + str(seg) + "s</span></div>"
            ...
            "background:linear-gradient(90deg," + col_timer + "aa," + col_timer + ");"
            "transition:width .95s linear'></div></div>"
            + ("<div style='color:#ef4444;font-size:.74rem;font-weight:700;margin-top:5px;"
               "text-align:center;animation:pulse 0.5s infinite'>¡Responde ya!</div>" if seg <= 8 else "")
            + "</div>",
            unsafe_allow_html=True
        )

        # Tarjeta de pregunta — tipografía NORMAL (sin pixel art)
        cat_color = {
        ...
        dif_color = {"facil":"#10b981","normal":"#f59e0b","dificil":"#ef4444"}.get(dif_preg,"#94a3b8")
        dif_label = {"facil":"Fácil","normal":"Normal","dificil":"Difícil"}.get(dif_preg,"")
        ...
        st.markdown(
            ...
            "border:1px solid " + dif_color + "44;border-radius:20px;padding:2px 10px;"
            "font-size:.68rem;font-family:Courier Prime,monospace'>" + dif_label + "</span>"
            "</div>"
            "<p style='color:#f1f5f9;font-size:1rem;font-weight:600;line-height:1.5;"
            "font-family:Inter,sans-serif;margin:0'>" + pregunta.get("q","") + "</p>"
            "</div>",
            unsafe_allow_html=True
        )

        opciones_letras = [chr(65+i) + ". " + op for i, op in enumerate(pregunta["ops"])]
    ...
    # ══════════════════════ FASE: RESULTADO PREGUNTA ══════════════════════════
        ...
        if correcta:
            ...
            cambios_html = " ".join(
                ...
                + IND_COLOR.get(k, ("#94a3b8",""))[1] + " " + IND_LABEL.get(k,"") + " "
            ...
            aviso_par = (" — Ronda par: penalización DOBLE (" + str(penalizacion) + " pts)") if es_par else ""
            pen_real  = penalizacion if es_par else (penalizacion // 2 if not es_par else penalizacion)
            # penalización estándar según config
            pen_real  = penalizacion
            nueva_ind = _aplicar_efectos(ind, {k: -pen_real for k in ind})
            actualizar_progreso(gid, nueva_ind["economia"], nueva_ind["medio_ambiente"],
                                nueva_ind["energia"], nueva_ind["bienestar_social"], ronda)
            st.markdown(
                ...
                "<p style='color:#fca5a5;margin-bottom:6px'>La correcta era: <b>" + texto_ok + "</b></p>"
                "<p style='color:#fca5a5;font-size:.84rem'>Todos los indicadores pierden "
                "<b>" + str(pen_real) + " puntos</b>" + aviso_par + ".</p></div>",
                unsafe_allow_html=True
            )
    ...
    # ══════════════════════════ FASE: EVENTO ══════════════════════════════════
        ...
        dif_cfg = DIFICULTADES.get(dif, DIFICULTADES["Normal"])
        pesos   = dif_cfg["eventos_peso"]

        if st.session_state.get("evento_ronda") is None:
            from config import EVENTOS_NEGATIVOS, EVENTOS_POSITIVOS
            pool   = (EVENTOS_NEGATIVOS * int(pesos["negativos"]*10) +
                      EVENTOS_POSITIVOS * int(pesos["positivos"]*10))
        ...
        st.markdown(
            ...
            + IND_LABEL.get(evento["indicador"], nom_ind) + "</span>"
            ...
            "<span style='color:" + col_ev + ";font-size:.9rem;margin-left:8px'>"
            + signo + str(evento["valor"]) + "</span>"
            "</div></div>",
            unsafe_allow_html=True
        )

        if st.button("Finalizar Ronda " + str(ronda) + "/" + str(TOTAL_RONDAS), use_container_width=True):
